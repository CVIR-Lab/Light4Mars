...
class SegLocalVisualizer(Visualizer):
    ...
    def add_datasample(
        ...
        if gt_img_data is not None and pred_img_data is not None:
            drawn_img = np.concatenate((gt_img_data, pred_img_data), axis=1)  # image add
        ...
        if show:
            ...
            self.show(drawn_img, win_name=name)

        if out_file is not None:
            mmcv.imwrite(mmcv.rgb2bgr(drawn_img), out_file)
        else:
            # self.add_image(name, drawn_img, step)
            self.add_image(name, drawn_img)
